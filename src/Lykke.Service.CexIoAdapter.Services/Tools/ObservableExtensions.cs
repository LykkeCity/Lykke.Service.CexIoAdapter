...
namespace Lykke.Service.CexIoAdapter.Services.Tools
{
    public static class ObservableExtensions
    {
        ...
        public static IObservable<IObservable<T>> RepeatShared<T>(this IObservable<T> source)
        {
            return Observable.Create<IObservable<T>>(async (obs, ct) =>
            {
                while (!ct.IsCancellationRequested)
                {
                    var shared = source.Publish().RefCount();
                    obs.OnNext(shared);
                    await shared.LastOrDefaultAsync().ToTask(ct);
                }
